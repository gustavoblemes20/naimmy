<?php
/**
 * Template Name: Miu Miu Home
 * Página inicial inspirada no estilo minimalista da Miu Miu
 */
...
<div class="hero-slider" id="heroSlider">
    <?php
    // Carregar slides do WordPress (posts com categoria 'slides' ou custom fields)
    ...
    // Se não houver slides customizados, usar slides padrão
    if (empty($slides)) {
        ...
        foreach ($default_slides as $index => $slide) {
            ...
            echo '<div class="slide ' . $active_class . '" style="background-image: linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url(' . $slide['background'] . ');">';
            echo '<div class="slide-content">';
            echo '<h1 class="slide-title">' . $slide['title'] . '</h1>';
            echo '<p class="slide-subtitle">' . $slide['subtitle'] . '</p>';
            echo '<a href="' . $slide['button_link'] . '" class="slide-button">' . $slide['button_text'] . '</a>';
            echo '</div>';
            echo '</div>';
        }
    } else {
        // Usar slides customizados do WordPress
        foreach ($slides as $index => $slide) {
            $active_class = $index === 0 ? 'active' : '';
            $background = get_post_meta($slide->ID, 'slide_background', true);
            $title = get_post_meta($slide->ID, 'slide_title', true) ?: $slide->post_title;
            $subtitle = get_post_meta($slide->ID, 'slide_subtitle', true);
            $button_text = get_post_meta($slide->ID, 'slide_button_text', true) ?: 'Saiba Mais';
            $button_link = get_post_meta($slide->ID, 'slide_button_link', true) ?: '#';
            
            echo '<div class="slide ' . $active_class . '" style="background-image: linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url(' . $background . ');">';
            echo '<div class="slide-content">';
            echo '<h1 class="slide-title">' . $title . '</h1>';
            echo '<p class="slide-subtitle">' . $subtitle . '</p>';
            echo '<a href="' . $button_link . '" class="slide-button">' . $button_text . '</a>';
            echo '</div>';
            echo '</div>';
        }
    }
    ...
    <div class="slide-controls">
        <?php
        $slide_count = !empty($slides) ? count($slides) : 3;
        for ($i = 0; $i < $slide_count; $i++) {
            $active_class = $i === 0 ? 'active' : '';
            echo '<button class="slide-dot ' . $active_class . '" data-slide="' . $i . '"></button>';
        }
        ?>
    </div>
</div>
